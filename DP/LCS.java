public class LCS {
    ...
    public static int lcs2(String str1, String str2, int n, int m, int dp[][]) {
        ...
        if (dp[n][m] != -1) {// agar -1 nahi hai means woh jagah ke liyeh solve kiya hua hai use direct
                             // return kardo
            return dp[n][m];
        }
        // otherwise un n m ki value ke liyeh solve karke dp me add kardo
        // same
        if (str1.charAt(n - 1) == str2.charAt(m - 1)) {
    ...
    public static void main(String args[]) {
        ...
        int n = str1.length();
        int m = str2.length();
        int dp[][] = new int[n + 1][m + 1];// initialize the dp for the memonization code
        for (int i = 0; i < n + 1; i++) {
            for (int j = 0; j < m + 1; j++) {
                dp[i][j] = -1;// with -1;
            }
